...
**Revision update.**  
We ran a zero-shot strict-OOD evaluation against three tissues absent from the training corpus: kidney (Tabula Sapiens, 3,406 cells), cerebellum (human brain atlas, 19,981 cells), and fetal gonadal tissue (3,342 cells including 294 Leydig + 3,048 adrenal cortex type I). All three tissues pass a strict leakage audit (zero mentions of `kidney / renal / nephron / cerebellum / cerebellar / Purkinje / testis / Sertoli / spermatogonia` across nine training-metadata files) and were ingested via the CellxGene Census API without any training-side changes. The production CLOP-DiT partially generalises: nearest-real-centroid accuracy is 3–6× random for structurally-familiar novel types (adrenal cortex type I 0.875, kidney epithelial cell 0.460) but at-or-below random for structurally-distinct novel types (Purkinje cells 0.00, granule cells 0.04, cerebellar interneurons 0.00). This characterises the model's zero-shot generalisation ceiling directly: the learned cross-type manifold extrapolates along familiar structural programs (epithelial, steroidogenic) but not along unfamiliar ones (GABAergic cerebellar neurons). Detailed results in `revision/experiments/lane_c_data/zero_shot_summary.md` and `zero_shot_results.json`. Expanded training coverage to neural and gonadal tissues remains explicit future work.
