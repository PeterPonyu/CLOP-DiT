...
Last updated: 2026-04-17 (targeted figure + layout fixes + dedupe: (a) panel-label helper in src/visualization/style.py upgraded to render BOLD UPPERCASE A/B/C (default fontsize 14→16pt, fontweight semibold→bold, parentheses dropped, internal upper-case); every add_panel_label call across the 25-figure manifest now shows a large prominent "A"/"B"/... instead of a small "(a)"/"(b)"/...; (b) figS_lane_a1 right-side within-family bars now pin their y-lim to the heatmap y-lim, so each bar sits on exactly the heatmap row it labels; (c) figS_lane_a2 panel labels moved from va="top" at y=1.04 (overlapping titles) to add_panel_label(x=-0.18, y=1.10) — clean separation; (d) figS_lane_b3 figsize shrunk 4.2→3.4 and subplots_adjust bottom 0.30→0.22 — eliminates the large blank stripe; (e) fig09a/fig09b outlier callouts now use axes-fraction direction logic (flip dx/dy based on quadrant) + annotation_clip=True — every label stays inside the plot rectangle; (f) removed byte-identical duplicate revision/manuscripts/diff/clop_dit_manuscript_tracked_changes.pdf (was a cp of .diff.pdf); SUBMISSION_CHECKSUMS.txt now references clop_dit_manuscript.diff.pdf as the canonical tracked-changes artefact; (g) added revision/manuscripts/diff/README.md explaining why abstract/intro prose shows as unchanged in the latexdiff PDF (body text unchanged between v1 and v2; only v2 additions render underlined); (h) removed 3 "Data source:" internal filesystem paths from supplementary figure captions in a prior round — confirmed persist post-rebuild with 0 leaks. Main manuscript 42→41 pages, diff 43→42, rebuttal 12, cover 3.)

Last updated (prior): 2026-04-17 (final polish pass: (a) 25-figure deep re-read — 21 PASS / 4 ADVISORY / 0 BLOCKER, report at final_figure_polish_2026-04-17.md; (b) confidential-info leak audit closed 3 BLOCKERs — removed internal "revision/experiments/..." filesystem paths from the 3 new supplementary-figure captions (Figs 13, 14, 15); post-fix grep confirms 0 internal paths / 0 AI-tool mentions / 0 Claude/Anthropic/ralph/codex/scivcd/autopilot strings across all 4 PDFs; report at confidential_leak_audit_2026-04-17.md; (c) added explicit "Primary target" statement to both Abstract and Introduction penultimate paragraph — "controllable, biologically faithful single-cell latent generation from structured metadata, measured jointly on type specificity, prompt-driven steering controllability, and within-type heterogeneity preservation"; (d) added a **Metrics glossary** paragraph at the end of §2.6 Evaluation Framework defining every reported metric in one line each (KNN top-1/top-5, Steering, DivR, FD, Coverage, Density, Centroid cosine, Linear classifier accuracy, r_var, Mantel r, SWD, Discriminator AUC, Cluster purity); (e) added a "New supplementary figures" bullet to the cover-letter Key Manuscript Changes explicitly naming Figures 13/14/15 and the refined Figures 9a/9b; (f) rebuilt manuscript 42→41 pages (tighter after caption cleanup); all four PDFs rebuild with 0 undefined refs / 0 "??" artefacts / 0 leaks; sha256sum -c green.)

Last updated (prior): 2026-04-17 (cover-to-cover read-through + BLOCKER fixes: (a) replaced six `\ref{fig:*}` in §Extended validation that all resolved to "Figure 11" with panel-lettered references (Figure~11(a)–(l)) so reviewers no longer see "Figures 11, 11, 11, 11, 11, and 11"; same fix for four `\ref{fig:*}` in §Ablation; (b) "(Figure 6) ... (Figure 6)" duplicate in §Conditioning Landscape clarified with panel letters (a–d vs e); (c) orphan "Table S2" replaced with `\ref{tab:s2_validation}` in §3.9 and Table 15 caption; (d) Author Contributions expanded to name Y.L., J.W., S.W. beyond Z.F.; (e) cerebellum-OOD numbers reconciled with authoritative JSON data — granule cells 0.045 → granule 0.00 + granule precursor 0.035, inhibitory interneurons 0.00 → 0.025 (both in manuscript Section 3.13 and rebuttal Comment 3.1); (f) rebuttal ZCA three-way table proto_acc column corrected from scale-mismatched 0.999/1.000/0.998 to matching manuscript 76.17/76.03/75.42%; (g) unresolved `??` cross-refs in rebuttal (R2.6 confusion heatmap, R3.2 boxplot, R3.3 forced-scarcity) and cover letter (Table 6, Figure 9, Table 8) replaced with hardcoded manuscript-side numerals; (h) cover-letter "more honestly" softened to "more transparently"; (i) rebuttal ZCA reworded "5% improvement" → "+4.2 pp absolute / ~5.2% relative" to remove units ambiguity. All four submission PDFs rebuilt (42/43/12/3 pages), sha256sum -c green. Audit trails: manuscript_readthrough_2026-04-17.md, rebuttal_readthrough_2026-04-17.md.)

Last updated (prior): 2026-04-17 (figure refinements + venue-neutral rename: (a) fig01a_architecture — shared-space dashed box was overlapping the z_0 / N(0,I) label; tightened shared_w 0.62→0.56 and anchored dit_x0 to shared_box right edge + 0.20 clearance; decoder dec_x0 now computed from ode_x+ode_w+0.12 so all Stage-2/3 boxes cascade correctly. Post-fix visual audit shows clean separation. (b) fig09a_variance_matching panel (d) and fig09b_gene_gene_correlation panel (h) — outlier callouts previously placed at fixed right-margin slots (0.83, 0.94), (0.83, 0.14), etc. caused leader lines that spanned ≥ 50% of panel width and crossed through data points. Replaced with xytext offset-point annotate() placements so leaders stay under ~15% of panel width; cleaned dead code. (c) **Venue-neutral rename**: `clop_dit_genes.{tex,pdf}` renamed to `clop_dit_manuscript.{tex,pdf}` across revision/manuscripts/v2_revision/, revision/manuscripts/v1_prerevision/, and revision/manuscripts/diff/ (all three tracked-changes outputs renamed to `clop_dit_manuscript.diff.{tex,pdf}` and `clop_dit_manuscript_tracked_changes.pdf`); `clop_dit_genes` referenced MDPI Genes, the retired venue; current target is Array (Elsevier). Updated src/utils/paths.py, scripts/pipeline/get_article_paths.py, tests/test_article_delivery.py defaults. Cleaned intermediate build artefacts (aux/log/out/bbl/blg/fls/fdb_latexmk) from all three manuscript directories. (d) All four submission PDFs rebuilt under new names; sha256sum -c green. (e) Rebuttal + cover letter unchanged in content, just rebuilt.)

Last updated (prior): 2026-04-17 (orphan-ref cleanup + 3 new supplementary figures + R2.5 reframing: (a) eliminated all 18 previously orphaned `\label{fig:*}` floats in the main manuscript — every figure defined with a label is now `\ref`'d at least once from body prose; post-patch Python audit returns 0 orphans (41 labels / 41 refs); (b) added three new reviewer-facing supplementary figures drawn from existing Lane A/B JSON artefacts: `figS_lane_a1_knn_family_heatmap.pdf` (R2.6, 10×10 family confusion heatmap + within-family error bars; 49.2% within-family headline), `figS_lane_a2_organism_stratified.pdf` (R3.2, 6-panel boxplots with all five Mann–Whitney p-values visualised), `figS_lane_b3_forced_scarcity.pdf` (R3.3, F1 trajectories under natural vs forced scarcity showing augmentation has real headroom only at forced scarcity); (c) strengthened R2.5 with a dedicated "stage-specific retrieval proxy" paragraph in §3.1 explicitly reframing the 87% train / ≈2.5% validation gap and pointing readers at the downstream KNN/steering metrics instead; (d) wired the three new figures into Appendix §A.1, the article_delivery.py manifest (22→25 figures), and rebuttal R2.6/R3.2/R3.3 via `\msref{Appendix~A.1}` pointers; (e) re-ran latexdiff to regenerate the tracked-changes manuscript; all four submission PDFs rebuilt with 0 undefined refs; main manuscript now 42 pages, rebuttal 12 pages. Reports: `revision/figure_fix_reports/comment_adequacy_audit_2026-04-17.md`.)

Last updated (prior): 2026-04-17 (deep cleanup + supplement-citation patch + rebuttal figure fix + whitespace tighten: (a) removed 6 stale old-numbered figure_fix_reports (fig25..fig30) via `git rm`; purged 4 outdated root-level docs (AUDIT_INDEX, IMPROVEMENT_PLAN, SUBMISSION_AUDIT_PEERJ_CS, ZENODO_WORKFLOW) that referenced the pre-revision PeerJ CS/MDPI venues; removed orphaned v2_revision/Definitions/ and v2 misplaced clop_dit_genes.diff.* build artefacts; removed v1_prerevision stale PeerJ CS cover letter; (b) ran a supplement-citation audit across all 15 completed experiments and patched the 5 GAPs: Lane B2 ZCA now 3-way (ZCA / centre+L2 / none) with quality 0.966/0.960/0.956 and pos-pair cosine 0.851/0.815/0.809; Lane B4 bridge now reports FD 0.230 vs 0.175 (+31%), coverage 0.079 vs 0.144 (-45%), centroid cosine 0.854 vs 0.929 (-8.1%); Lane B3 forced-scarcity (baseline 0.50 → oversampling 0.783/0.866, hybrid 0.772/0.835) added to Results §Rare Cell Augmentation; encoder-bottleneck trio (cap-increase, HVG count, raw-vs-latent compression) consolidated into Discussion §Where the bottleneck lives; (c) regenerated figS_lane_c_zero_shot.pdf with explicit (a)/(b)/(c) panel labels and reduced left margin 0.42 → 0.24 — eliminating the large left whitespace the user flagged; rebuttal now embeds the figure with `\centering` + descriptive caption; (d) tightened both the manuscript and rebuttal preambles (textfloatsep 6pt→4pt, topfraction 0.95→0.97, bottomfraction 0.85→0.93, added parskip 3pt±1pt, shrunk tcolorbox before/after skip 8pt→4pt and \bigskip→\medskip globally) — rebuttal page count dropped 14→12; (e) all four submission PDFs rebuilt; sha256sum -c green. Reports: `revision/figure_fix_reports/{deep_cleanup_inventory,supplement_citation_audit}_2026-04-17.md`.)

Last updated (prior): 2026-04-17 (visual figure audit + prose sweep + legacy cleanup: (a) visual read of all 22 article figures found one BLOCKER on fig08a panel (e) where 0.00 P/R/F1 values rendered as near-invisible black-on-black on the inferno colormap — fixed by adding `matplotlib.patheffects` stroke outline to every cell label and bolding the font; (b) **R2.1 abstract blocker fixed** — the `\mdpiemitheader` emitter had been rendering an empty abstract because `\begin{abstract}` was colliding with the renewed `\abstract` macro, so the reviewer-facing abstract was never appearing on page~1; the preamble now emits the abstract as an indented quoted paragraph and the abstract text was restored with R2.1 background + challenge framing; (c) the manuscript `\Title / \Author / \address / \corres / \abstract / \keyword` metadata block was restored after the MDPI → neutral-preamble swap (commit `cddd7cb`) had accidentally dropped it; (d) all legacy submission variants cleaned — gitignored `articles/`, `articles_elsevier/`, `articles_elsevier copy/`, `articles.zip`, `LaTeX_Template_for_PeerJ_Journal_Submissions/`, repo-root `cover_letter*.{md,pdf,tex}`, and the `.mdpi_backup` were deleted from disk (all were already `.gitignore`d and untracked); tracked `figure_subpanels_archive/` removed via `git rm`; stale `revision/manuscripts/v2_revision/cover_letter_peerj_cs.*` removed (venue is Array/Elsevier per the active cover letter); (e) latexdiff re-run to regenerate the tracked-changes diff against a neutralised v1; all four submission PDFs rebuilt with `sha256sum -c revision/SUBMISSION_CHECKSUMS.txt` green. Reports: `revision/figure_fix_reports/visual_figure_audit_2026-04-17.md`, `prose_audit_2026-04-17.md`, `legacy_variants_audit_2026-04-17.md`.)

Last updated (prior): 2026-04-17 scivcd revision sweep — fig09a/9b/S_lane_c layout truncation CRITICALs fixed; all 22 article PDFs regenerated under `--adaptive --vs-baseline` with 0 new CRITICAL vs baseline; figures synced into v2_revision and diff manuscript dirs; rebuttal_letter.tex wired to name Figures 9a, 9b, S1, S2 in R2.8/R2.9/R2.11/R3.3; all three LaTeX products rebuilt clean; submission checksums refreshed. Close-out report at `revision/figure_fix_reports/scivcd_revision_closeout_2026-04-17.md`.
