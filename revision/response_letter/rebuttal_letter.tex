...
In the revision, we have completed \textbf{five post-hoc analyses} (Lane~A: KNN error taxonomy, organism stratification, abundance--fidelity correlation, gene-level variance quantification, rare-cell failure mechanism), \textbf{four partial-retrain experiments} (Lane~B: Gaussian variance baselines, ZCA whitening ablation, augmentation strategy sweep with forced-scarcity control, and CLOP-to-full-pipeline bridge), \textbf{one encoder comparison study} (Lane~D), and \textbf{a zero-shot strict-OOD evaluation} on kidney, cerebellum, and fetal gonadal tissue ingested via the CellxGene Census (Lane~C, addressing Comment~3.1 directly). Together, these converge on a single mechanistic diagnosis: CLOP-DiT's principal limitation is \textbf{latent-stage under-dispersion of within-type heterogeneity}, not decoder collapse, not species imbalance, not data scarcity, and not a correctable-by-mixing downstream artefact; the zero-shot Lane~C results extend this picture by showing the model partially generalises along familiar structural programs (steroidogenic, epithelial) but not along unfamiliar ones (GABAergic cerebellar neurons).
...
\begin{revisionupdate}
\textbf{Lane~C --- Zero-shot strict-OOD evaluation (completed).}
...
\noindent\textbf{Manuscript changes:}
\begin{itemize}
  \item Added the zero-shot strict-OOD paragraph with per-tissue metrics. \msref{Section~3.11}
  \item Added Supplementary Table with per-cell-type nearest-centroid accuracies and FD. \msref{Supplement~S13}
  \item Kept the interpolation-vs-extrapolation clarification on the 8-study held-out split. \msref{Section~3.3}
