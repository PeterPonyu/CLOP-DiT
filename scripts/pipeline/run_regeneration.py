...
def run_fig04_composed():
    """Step 3 of single-producer migration: produce fig04_composed.pdf
    (.omc/plans/single-producer-architecture-2026-04-20.md).

    Dual-published alongside the legacy fig04a/fig04b slices: fig08_markers and
    fig09_expression_corr (the article-Fig-4-A and article-Fig-4-B sources —
    source-file numbering diverged from article numbering pre-revision) still
    emit their per-slice PDFs unchanged; this composite adds a single
    4-row x 2-col gridspec PDF with aligned panel labels and shared fonts for
    the article Figure 4.
    """
    log.info("── Generating fig04_composed.pdf ──")
    try:
        from src.visualization.fig04_composed import plot_fig04_composed
        result = plot_fig04_composed(
            output_dir=str(FIG_DIR),
            dpi=300,
            save=True,
        )
        plt.close("all")
        if result is None:
            log.warning("fig04_composed skipped (no expression data)")
            return None
        log.info("fig04_composed: %s (%s)", result, "exists" if result.exists() else "MISSING")
        return result if result.exists() else None
    except Exception as e:
        log.error("fig04_composed failed: %s", e, exc_info=True)
        return None
...
def main():
    ...
    # 7e. Step 3 — single-producer Fig 4 composite (dual-published with legacy
    # fig04a/fig04b slices from fig08_markers/fig09_expression_corr — the
    # article-Fig-4 sources, despite their source-file numbering).
