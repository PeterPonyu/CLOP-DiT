...
- **Default output:** Panel and figure generators use `output_dir="results/figures"` by default so standalone runs write to the same location as the pipeline.
...
2. **No panel labels in content**: Subplot titles use descriptive text only (e.g. "Loss Convergence"). The article caption provides figure labelling.
3. **Layout-aware long labels**: When y-labels are long (cell type names > 20 chars), panel code automatically reduces subplots per row (from 3 → 2 or 1×3 → 2×2) for readability.
4. **Font sizes**: Base font sizes in `style.py` are calibrated for LaTeX at `composed_scale=0.95` (full-width figures). Minimum effective size is 5.5pt per VCD policy.
5. **VCD + auto-refine**: `save_with_vcd` runs 30 detection passes across 4 layers. `auto_refine.py` uses weighted scoring and category-specific action appliers to react to issues.
...
6. **Tighter layout**: `GRIDSPEC_TIGHT` in `src/visualization/style.py` uses `wspace=0.45`, `hspace=0.95`; `savefig.pad_inches=0.15`.
7. **Architecture figure**: Generated by `scripts/generate_architecture_figure.py`. Placed as Figure 1 in the article.
...
- **Default output:** Panel and figure generators default to `output_dir="results/figures"` (string). The central default is also defined as `FIG_DIR` in `src/utils/paths.py`, which respects `CLOPDIT_FIG_DIR` (and `CLOPDIT_RESULTS_DIR`) so the output directory can be overridden at runtime. Scripts and visualizers may be updated to pass `paths.FIG_DIR` for consistency.
...
|------------|-------------|
| **VCD** | One known warning remains (Panel S: low-contrast text on benchmark heatmap). Target is 0 warnings for submission; info-level issues are documented and accepted. |
| **Pipeline order** | Steps must run in order; some figures depend on earlier steps (e.g. Fig 11 is composed from panels J and K produced in steps 3 and 7). |
...
- **Warnings**: 1 total (Panel S: low-contrast text at 1.6–2.8:1 on benchmark heatmap)
...
After the layout restructuring, all panels achieve 0 VCD warnings except Panel S (1 low-contrast warning for heatmap value annotations). Info-level detections are structural and acceptable.
