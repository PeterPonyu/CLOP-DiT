...
\caption{Metrics dashboard summarizing conditional generation quality. KNN accuracy ($37\times$ random), steering accuracy (81\%), diversity ratio, linear classifier accuracy, and Fr\'{e}chet distance are shown across generation configurations. The collapse of unconditional generation to random baselines validates that text conditioning drives all cell-type specificity.\label{fig:metrics}}
\end{figure}
...
\caption{Per-type fidelity and text--cell alignment. \textbf{Top row}: Per-type centroid cosine similarity ranked across cell types with the hardest types annotated; Fr\'{e}chet outlier profile with the global mean marked; and abundance--fidelity scatter in which bubble size reflects Fr\'{e}chet distance and colour encodes the per-type diversity ratio. Together these panels expose the heterogeneity of generation quality across cell types instead of collapsing it to one mean score. \textbf{Bottom row}: 69$\times$69 cosine similarity heatmap between text and cell centroids (strong diagonal dominance), per-type alignment bars, and diagonal vs.\ off-diagonal distribution comparison.\label{fig:fidelity-alignment}}
\end{figure}

\subsection{Marker Gene Fidelity}

Figure~\ref{fig:markers} compares the expression of canonical marker genes between real and generated cells, demonstrating that CLOP-DiT preserves cell-type-specific expression signatures.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/panel_n_marker_gene_comparison.pdf}
\caption{Marker gene comparison. Expression of canonical marker genes in real vs.\ generated cells across selected cell types. Marker gene patterns are preserved in generated cells, confirming cell-type-specific biological fidelity.\label{fig:markers}}
\end{figure}
...
\caption{Expression correlation analysis. Per-gene Pearson correlation between real and generated mean expression profiles, stratified by cell type. High correlation confirms that the scGPT decoder faithfully reconstructs gene-level signatures from DiT-generated latent vectors.\label{fig:expr-corr}}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/panel_i_expression_analysis.pdf}
\caption{Expression distribution analysis. Coefficient of variation scatter, expression range with percentile bands, per-cell variability distribution, and top variable gene std ratios. Norm comparison: generated 20.97 vs.\ real 20.89.\label{fig:expr-analysis}}
\end{figure}

\subsection{Conditioning Landscape and Diversity Trade-Offs}

Figure~\ref{fig:cond-umap} visualizes how text conditions distribute in the CLOP projection space and how the resulting generated cell clouds cluster accordingly. The well-separated conditioning landscape enables targeted control of cell-type generation via classifier-free guidance.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/panel_m_conditioning_umap.pdf}
\caption{Conditioning landscape. UMAP of the CLOP condition space showing how text conditions and their corresponding generated cell clouds distribute, visualizing the text-to-cell steering mechanism.\label{fig:cond-umap}}
\end{figure}
...
\caption{Diversity diagnostics. Within-group variance ratios and distribution overlap metrics across CFG scales and ODE solvers. KNN accuracy saturates at CFG $\approx$ 2.0--3.0 while diversity decreases monotonically.\label{fig:diversity}}
\end{figure}
...
\caption{Composite benchmark. (\textbf{a})~Normalized metrics heatmap (methods $\times$ eleven metrics; green = high performance). (\textbf{b})~Composite score aggregating KNN accuracy, steering, diversity ratio, and linear separability (higher = better). (\textbf{c})~Grouped bar chart of key metrics across methods. (\textbf{d})~Bootstrap 95\% confidence intervals for composite scores. CLOP-DiT achieves a composite score of 0.668, confirming its advantage as an integrated text-conditioned generation system.\label{fig:benchmark}}
\end{figure}
