...
Existing approaches to single-cell data generation include variational autoencoders such as scVI~\cite{ref-scvi} and scGen~\cite{ref-scgen}, geometric deep generative models~\cite{ref-scdiff}, and more recently, large-scale single-cell foundation models such as Geneformer~\cite{ref-geneformer} and scBERT~\cite{ref-scbert} that leverage transfer learning for downstream prediction. These methods condition on categorical cell-type labels or perturbation metadata, but to our knowledge none accept structured natural-language biological descriptions as conditioning input. In parallel, contrastive vision--language models such as CLIP~\cite{ref-clip} have demonstrated that aligning two modalities into a shared embedding space enables powerful zero-shot transfer, with the SimCLR~\cite{ref-simclr} and MoCo~\cite{ref-moco} frameworks establishing the self-supervised contrastive paradigm. Domain-specific variants such as BiomedCLIP~\cite{ref-biomedclip} and SigLIP~\cite{ref-siglip} extend this paradigm to biomedical data. Meanwhile, Diffusion Transformers (DiTs)~\cite{ref-dit} have emerged as state-of-the-art conditional generators, building on latent diffusion models~\cite{ref-latent-diffusion}, and flow matching~\cite{ref-flow-matching} provides a simulation-free training objective that simplifies the diffusion framework. The Transformer architecture~\cite{ref-transformer} underpins both the DiT generator and the frozen encoders.
...
\caption{Per-type fidelity and text--cell alignment. (\textbf{a})~Per-type centroid cosine similarity ranked across cell types, (\textbf{b})~Fr\'{e}chet outlier profile with the global mean marked, and (\textbf{c})~abundance--fidelity scatter (bubble size $\propto$ Fr\'{e}chet distance, colour encodes diversity ratio). Cell-type names are mapped by rank position in the supplementary materials. (\textbf{d})~69$\times$69 cosine similarity heatmap between text and cell centroids (strong diagonal dominance), (\textbf{e})~per-type alignment bars, and (\textbf{f})~diagonal vs.\ off-diagonal distribution comparison.\label{fig:fidelity-alignment}}
\end{figure}
...
\caption{Noise--scale trade-off and expression diversity. (\textbf{a})~Fr\'{e}chet distance, centroid cosine, and diversity ratio as a function of noise scale $\varepsilon$ at CFG = 1.5, with the production configuration ($\varepsilon = 0.03$) highlighted. (\textbf{b})~Gene-expression-level diversity in generated vs.\ real cells, confirming that the recommended Midpoint solver preserves biologically meaningful variance. (\textbf{c})~Violin comparison of sampled pairwise cosine distributions (real--real, generated--generated, and real--generated) for the six cell types with the largest intra-type cosine shifts.\label{fig:diversity-tradeoff}}
\end{figure}
...
\bibitem{ref-ema}
Tarone,~R.E. The use of the harmonic mean in the calculation of average speeds. {\em Biometrics} {\bf 1976}, {\em 32}, 671--674.
