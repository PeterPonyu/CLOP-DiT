...
We present CLOP-DiT, a two-stage pipeline that combines \textbf{C}ontrastive \textbf{L}anguage--\textbf{O}mics \textbf{P}retraining (CLOP) with a conditional \textbf{Di}ffusion \textbf{T}ransformer (DiT) to generate single-cell gene expression profiles from text descriptions (Figure~\ref{fig:architecture}). CLOP aligns BiomedBERT~\cite{ref-biomedbert} text embeddings and scGPT~\cite{ref-scgpt} cell embeddings into a shared 256-dimensional space; DiT then performs conditional flow matching in the scGPT latent space, guided by CLOP-projected text conditions; and a frozen scGPT decoder maps generated latent vectors back to gene expression. Across 100 evaluated cell types, CLOP-DiT achieves a KNN classification accuracy of 36.9\% (37$\times$ random chance), a steering accuracy of 81.0\%, and a diversity ratio of 0.93, demonstrating that text conditioning drives biologically meaningful, type-specific generation.
...
\begin{figure}[H]
\centering
% Full width; keep aspect so right side (decoder, gene expr) is not truncated
\includegraphics[width=\textwidth,height=0.38\textheight,keepaspectratio]{figures/fig_architecture.pdf}
...
\subsection{CLOP: Contrastive Language--Omics Pretraining}
...
\subsection{DiT: Conditional Flow Matching with Diffusion Transformer}
...
The biological evidence is organized by module and figure family. Figures~\ref{fig:markers}--\ref{fig:expr-analysis} are produced by the biological-validation stack (\texttt{scripts/08\_biological\_validation.py} and \texttt{src.evaluation.biological\_validation}) and establish marker-level and gene-level fidelity. Figures~\ref{fig:diversity}--\ref{fig:diversity-tradeoff} are produced by \texttt{scripts/diversity\_diagnostics.py} and \texttt{scripts/conditioning\_analysis.py} and provide the main robustness readout for guidance and diversity. Figures~\ref{fig:baselines}--\ref{fig:benchmark} are produced by \texttt{src.evaluation.model\_benchmarking}, while Figures~\ref{fig:downstream-pq} and~\ref{fig:downstream-r} come from \texttt{src.evaluation.downstream\_biology} and test whether generated cells remain useful in downstream single-cell workflows.
...
Figures~\ref{fig:expr-corr} and~\ref{fig:expr-analysis} examine the fidelity of generated expression profiles at the gene level. The expression correlation analysis (Figure~\ref{fig:expr-corr}) quantifies the per-gene Pearson correlation between real and generated mean expression across cell types. The expression analysis (Figure~\ref{fig:expr-analysis}) provides a broader view of expression distribution properties, including norm matching (generated mean norm 20.97 vs.\ real 20.89) and per-dimension statistics. Together they localize biological meaning in gene-wise means, variances, and marker-level residuals rather than only in embedding geometry.
...
\caption{Downstream validation: clustering and classifier alignment. \textbf{Top row}: Leiden clustering on combined real + generated data with UMAP overlay, the lowest-mixing cell types, and compact cluster-alignment gauges (ARI, NMI, cluster purity, mean $k$-NN mixing). \textbf{Bottom row}: Cell-type classification confusion matrix (real-trained on generated), a precision/recall/F1 heatmap restricted to the hardest 15 cell types, and the real-vs.-generated discriminator ROC curve. This layout exposes which cell types are genuine downstream failures versus near-perfect transfers.\label{fig:downstream-pq}}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/panel_r_de_concordance.pdf}
\caption{DE concordance. Effect-size-weighted real-vs.-generated log-fold-change scatter for the leading contrast, with point size proportional to average absolute effect size and colour indicating adjusted significance; sign-disagreement genes are outlined. A compact summary heatmap reports Pearson/Spearman correlation, Jaccard overlap of top DE genes, and sign agreement across the three biologically meaningful contrasts (CD8 vs.\ CD4 T, resident macrophage vs.\ monocyte, epithelial vs.\ fibroblast), replacing the previous redundant grouped-bar summary.\label{fig:downstream-r}}
\end{figure}
