...
def plot_discriminator_analysis(output_dir=None, dpi=300):
    ...
    layout = bind_figure_region(fig, (0.10, 0.16, 0.98, 0.88))
    left, mid, right = layout.split_cols([1.0, 1.0, 1.2], wspace=0.44)

    # Panel (h): Separability driver summary (mean vs variance)
    ax = left.inset(right=0.02).add_axes(fig)
    add_panel_label(ax, 'h', x=-0.12, y=1.14)

    # Bar chart: mean vs variance contribution
    labels_bar = ["Mean shift", "Variance shift"]
    ...
    ax.text(0.5, 0.95, f"Primary driver: {driver}",
            ...
            fontsize=FONT_SMALL, fontweight="bold", color=COLORS["annotation_dark"])
    style_axes(ax, "bar", ylabel="|Pearson r| with discriminator coefs",
               title="What Drives Separability?")

    # Panel (i): Coefficient concentration
    ax2 = mid.inset(left=0.05, right=0.02).add_axes(fig)
    add_panel_label(ax2, 'i', x=-0.12, y=1.14)

    conc = coef.get("concentration", {})
    x_labels = ["Top 10", "Top 20", "Top 50"]
    ...
    ax2.set_ylim(0, 100)
    style_axes(ax2, "bar", ylabel="% of total discriminative weight",
               title="Dimension Concentration")

    # Panel (j): Per-type AUC bar chart
    ax3 = right.inset(left=0.05, right=0.01).add_axes(fig)
    add_panel_label(ax3, 'j', x=-0.10, y=1.14)
    ...
    ax3.set_yticklabels(thin, fontsize=8)
    ax3.invert_yaxis()
    ...
    ax3.legend(fontsize=FONT_ANNOTATION, frameon=False, loc="lower right")
    style_axes(ax3, "bar", xlabel="Discriminator AUC",
               title="Per-Type Real/Gen Separability")
