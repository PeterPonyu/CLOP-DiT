...
def _draw_metrics_row(
    ...
    if gen_metrics or expr_metrics:
        ...
        if gen_metrics:
            ...
            bar_labels.append("FD\n(inv)"); bar_vals.append(fd_score)
            bar_labels.append("Coverage"); bar_vals.append(gen_metrics.get("Coverage", 0))
            bar_labels.append("Centroid\nCos"); bar_vals.append(gen_metrics.get("Centroid Cos", 0))
        if div_metrics:
...
def _draw_alignment_row(
    ...
    ax_j.legend(fontsize=10, frameon=False, loc="upper center",
    ...
    ax_j.text(
        ...
        fontsize=10, va="bottom", ha="right", color=COLORS["annotation_dark"],
...
def plot_fig03_composed(
    ...
    *_path              : JSON artefact paths (default to ``results/*.json``).
    ...
    row1, row2, row3 = layout.split_rows([1.00, 0.95, 1.05], hspace=0.48)

    # Row 1: 4 equal-weight columns (a/b/c/d).
    r1 = row1.split_cols([1.0, 1.0, 1.0, 1.0], gap=[0.045, 0.045, 0.045])
    ...
    ax_d = r1[3].inset(left=0.040, right=0.012).add_axes(fig)
