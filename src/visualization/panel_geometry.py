...
def apply_layout_rect(
    ...
    *,
    ...
    fig._clop_layout_rect = (left, bottom, right, top)
    fig._clop_layout_managed = True
    return fig._clop_layout_rect
...
def set_export_pad_inches(fig, pad_inches: float = DEFAULT_EXPORT_PAD_INCHES) -> float:
    ...
    fig._clop_export_pad_inches = float(pad_inches)
    return fig._clop_export_pad_inches
...
def get_export_pad_inches(fig, fallback: float = DEFAULT_EXPORT_PAD_INCHES) -> float:
    ...
    return float(getattr(fig, "_clop_export_pad_inches", fallback))
...
def mark_export_artists(fig, artists: Iterable[object]) -> None:
    ...
    current = list(getattr(fig, "_clop_export_artists", []) or [])
    current.extend(list(artists))
    fig._clop_export_artists = current